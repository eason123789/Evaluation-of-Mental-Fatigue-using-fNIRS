...
from gtheory.utils.misc import filter_path
...
def extract_delete_zip():
    ...
    from gtheory.download import extract_remove_zip
    pt = cf.root_path()
...
def load_to_epoch(**kwargs):
    from gtheory.eeg import (load_raw_to_epochs)
    from gtheory.utils.misc import sort_path
    from glob import glob
...
def cal_reaction_time(**kwargs):
    from gtheory.task_state import reaction_time
    from gtheory.utils.misc import sort_path
    from joblib import Parallel, delayed
    from glob import glob
    from tqdm import tqdm
...
def remove_overlap_events(**kwargs):
    ...
    '''
    from glob import glob
    from gtheory.utils.misc import sort_path, drop_overlap_event
    from tqdm import tqdm
    from gtheory import config as cf
    pt = cf.root_path(**kwargs)
...
def filter_epoch_to_fulfiled_condition(**kwargs):
    ...
    from joblib import Parallel, delayed
    from gtheory.utils.misc import sort_path
    from gtheory.eeg import drop_epoch_based_condition
    pt = cf.root_path(**kwargs)
...
def get_cluster_model(**kwargs):
    from glob import glob
    from gtheory import config as cf
    from gtheory.cluster import fit_kmeans
    from gtheory.utils.misc import sort_path
    from tqdm import tqdm
    import pandas as pd
...
def cluster_assignment(**kwargs):
    ...
    from gtheory import config as cf
    from tqdm import tqdm
    from gtheory.cluster import predict_kmeans_model
    from gtheory.utils.misc import sort_path
    import pandas as pd
    pt = cf.root_path(**kwargs)
...
def get_cluster_per_subject_report(**kwargs):
    ...
    from gtheory import config as cf
    from gtheory.utils.misc import sort_path
    from gtheory.utils.dataframe import save_list_df_multiple_sheets
...
def make_csdx(**kwargs):
    ...
    from gtheory.utils.misc import sort_path
    from tqdm import tqdm
    from gtheory.eeg import (make_csd)
    pt = cf.root_path(**kwargs)
    njob = 1
...
def _epoch_to_epochs(fband_window_max,**kwargs):
    ...
    from gtheory.utils.misc import sort_path
    from tqdm import tqdm
    from gtheory.eeg import (sliding_epoch_to_epochs)
...
def _make_connectivity(fband_window_max,**kwargs):
    ...
    from gtheory.utils.misc import sort_path
    from tqdm import tqdm
    from gtheory.eeg import (get_spectral_connectivity)
    kwargs['fband']=fband_window_max
    pt = cf.root_path(**kwargs)
    njob = 1
...
def cal_flexibility():
    ...
    # from gtheory.utils import between_list_diff_index
    # from gtheory.utils.nparray import slice_array_with_list
    from gtheory.grapht import get_community_each_epoch
    sbj1_path = sort_path(glob(f"{pt['dir_root']}/*/*/connectivity_result.npz"))
...
def assign_plot_flexibility():
    ...
    from tqdm import tqdm
    import pandas as pd
    from gtheory.grapht import flexibility_consec_time_windows
    from gtheory.utils import assign_pair
