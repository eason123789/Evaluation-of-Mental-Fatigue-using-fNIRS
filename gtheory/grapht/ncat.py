...
Extracted from : https://github.com/nangongwubu/Python-Version-for-Network-Community-Architecture-Toobox/blob/master/ncat.py
"""
import sys

import numpy as np
from netneurotools.modularity import zrand as zrandx
...
def integration(MA,systemByNode):
    ...
    # Modify by rpb for speed and simplicity
    I=[np.nanmean(MA[i,systemByNode!=systemByNode[i]]) for i in range(systemByNode.size)]
...
def consensus_similarity(C):
    '''
    RPB MADE CHANGES SUCH THAT, WE USED ZRAND FROM NETNEUROLAB
    ...
    Outputs:    consensus,      consensus partition
                ...
                average_npar: average of zrand over 100 optimization. add by rpb
    Remark:
            We used zrandx which extract from netneurolab instead of the original zrand. THe original zrand repeatedly
            produce and error.
    _______________________________________________
    Marcelo G Mattar (08/21/2014)
    '''
    ...
    for i in range(npart):
        for j in range(i+1,npart):
            ...
            pairwise_simm[i,j] =zrandx(C[i,:],C[j,:]) #we used the netneurotools as they r much better
    ...
    # zrand average over 100x optimization: add by rpb
    average_npar=average_pairwise_simm/npart
    ...
    consensus = C[I,:]
    consensus_simm = X
    return (consensus, consensus_simm, pairwise_simm,average_npar)

def consensus_iterative(C,gamma):
    '''
    CONSENSUS_ITERATIVE     Construct a consensus (representative) partition
    using the iterative thresholding procedure
    ...
    S2,Q2,lAlambda=multislice_static_unsigned(X_new3, gamma)
    # for i in range(npart):
        # print (i)

        # [S2(i,:) Q2(i)] = multislice_static_unsigned(X_new3,1)

        # [S,Q] = bct.community_louvain(W=X_new3,gamma=gamma)
        # S2[i,:] = S
        # Q2[i]  = Q
