...
for foldername in os.listdir(input_folder):
    ...
    for i in range(n_epochs):
        ...
        end = min(i * epoch_duration + 10, raw.times[-1])

        epoch_data = raw.copy().crop(tmin=start, tmax=end).get_data()

        # Ensure consistent length for epoch_data by padding or truncating
        expected_length = int(10 * raw.info['sfreq'])
        if epoch_data.shape[1] < expected_length:
            padding = np.zeros((epoch_data.shape[0], expected_length - epoch_data.shape[1]))
            epoch_data = np.concatenate([epoch_data, padding], axis=1)
        elif epoch_data.shape[1] > expected_length:
            epoch_data = epoch_data[:, :expected_length]
